...
serve(async (req) => {
  ...
  try {
    ...
    const provided_password_hash = await sha256(host_password + party_code);

    const is_match = provided_password_hash === data.host_password_hash;
